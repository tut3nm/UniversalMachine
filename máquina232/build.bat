...
REM                Resultado:  dist\ConfiguradorPlanta.exe
...
echo.
echo === Generando info de compilacion ===
for /f "delims=" %%D in ('%PYEXE% -c "import datetime; print(datetime.date.today().isoformat())"') do set "BUILD_DATE=%%D"
> "src\_build_info.py" (
  echo # Generado por build.bat - no editar a mano, no se commitea.
  echo FECHA_COMPILACION = "%BUILD_DATE%"
)
